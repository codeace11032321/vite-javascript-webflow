    ...
    onSnapshot(usersQuery, (querySnapshot) => {
      ...
      querySnapshot.forEach(async (doc) => {
        ...
        profileElement.innerHTML = `
          <img src="${userProfile.profilePicUrl || 'default_image_url'}" alt="${userProfile.name}" style="width: 40px; height: 40px; border-radius: 50%;" />
          <span>${userProfile.name}</span>
          <span class="status">${userProfile.isOnline ? 'Online' : 'Offline'}</span>
        `;
...
const sendMessage = async (e) => {
  ...
  const photoURL = userProfile ? userProfile.profilePicUrl : 'https://cdn-icons-png.flaticon.com/512/2202/2202112.png';
  ...
  try {
    await addDoc(collection(firestore, `messages/${user.uid}/${recipientUid}`), {
      text: messageText,
      createdAt: new Date(),
      uid: user.uid,
      photoURL: photoURL,
    });
    await addDoc(collection(firestore, `messages/${recipientUid}/${user.uid}`), {
      text: messageText,
      createdAt: new Date(),
      uid: user.uid,
      photoURL: photoURL,
...
const listenForMessages = async (recipientUid) => {
  ...
  onSnapshot(messagesQuery, (querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      messageElement.innerHTML = `
        <img src="${msg.photoURL || 'default_image_url'}" alt="User Avatar" id="user-pfl" style="width: 40px; height: 40px; border-radius: 50%;" />
        <p>${msg.text}</p>
      `;
      messagesList.appendChild(messageElement);
    });
